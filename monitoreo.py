...
def crear_db():
    try:
        ...
        cursor = con.cursor()
        cursor.execute(
            "CREATE TABLE sitios(url text PRIMARY KEY, description text, request_type text, monitoring_interval INTEGER,enabled INTEGER)")
...
def crear_sitio(url, descripcion, intervalo, tipo):
    try:
        con = sqlite3.connect(DB)
        cursor = con.cursor()
        cursor.execute('''insert into sitios (url,description,monitoring_interval,request_type,enabled) VALUES (?,?,?,?,1)''',
                       (url, descripcion, intervalo, tipo))
        con.commit()
...
def crear_log(url, start_time, end_time, elapsed, latency, status, resultado):
    try:
        con = sqlite3.connect(DB)
        cursor = con.cursor()
        cursor.execute('''insert into registro (url,start_time,end_time,elapsed,latency,status,result) VALUES (?,?,?,?,?,?,?)''',
                       (url, start_time, end_time, elapsed, latency, status, resultado))
        con.commit()
        ...
        try:
            if tipo.lower() == 'post':
                ...
                crear_log(url, str(start_time), str(end_time), str(
                    elapsed), latency, status, _parse_result_code(status))
            ...
            end_time = time.strftime(
                '%Y-%m-%d %H:%M:%S', time.localtime(end_time))
            crear_log(url, str(start_time), str(end_time),
                      str(elapsed), 408, _parse_result_code(408))
